...
def _evaluate(db: Session, session: SessionRecord) -> None:
    ...
    personal.update(device_id=session.device_id, target_count=features.unique_target_count, sensitive_reads=features.sensitive_resource_reads, after_hours=features.after_hours_score, risk_score=risk.score)
